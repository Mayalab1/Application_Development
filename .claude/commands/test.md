---
name: test
description: Valide les acceptance criteria via tests
triggers:
  - "teste"
  - "valide"
  - "tester"
...
Utilisez `--task TASK_ID` pour spécifier la tâche à tester (ex: `--task F001_T003`).
...
- **Patterns codebase** : stratégies de test efficaces
- **Gotchas** : cas limites difficiles à détecter
- **Edge cases** : scénarios de test à ajouter systématiquement

Format pour rapport de test (append-only) :
```
---
[Date] - Test [Task ID]
Résultat: PASS/FAIL
Tests exécutés: N total, N passed, N failed
Learnings:
- Pattern test: [stratégie de test à reproduire]
- Edge case: [cas limite découvert]
- Gotcha: [piège subtil dans les tests]
---
```

**IMPORTANT** : Toujours ajouter les nouveaux résultats, ne jamais effacer les rapports précédents.
