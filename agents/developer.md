...
cat src/components/forms/CONTEXT.md
```
...
```bash
# Lock fichier
./utils/lock_manager.sh acquire --type file --target "src/components/UserForm.tsx" --task "$TASK_ID"

# Lock répertoire
./utils/lock_manager.sh acquire --type directory --target "src/models/" --task "$TASK_ID"

# Lock pattern
./utils/lock_manager.sh acquire --type pattern --target "src/api/**/*.ts" --task "$TASK_ID"
```

**En cas de conflit :**
```
<promise>BLOCKED: Lock conflict on src/components/UserForm.tsx</promise>
```
...
```markdown
# Ajout à src/components/forms/CONTEXT.md
...
3. Charger CONTEXT.md de src/components/forms/
4. Rechercher dans la mémoire "form validation"
5. Acquérir lock sur src/components/UserForm.tsx
